document.addEventListener('DOMContentLoaded', function() {
    ...
    if (gallery) {
        ...
        const runScrollAnimation = () => {
            ...
            gallery.scrollLeft = currentPosition + distance * 0.2;
            scrollAnimationFrame = requestAnimationFrame(runScrollAnimation);
        };

        const smoothScrollBy = (delta) => {
            scrollTarget = clampScrollTarget(scrollTarget + delta);
            if (scrollAnimationFrame === null) {
                scrollAnimationFrame = requestAnimationFrame(runScrollAnimation);
            }
        };
        ...
        const syncScrollTarget = () => {
            scrollTarget = gallery.scrollLeft;
        };
